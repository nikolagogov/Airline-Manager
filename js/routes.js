...
import { checkBankruptcy, updateStatistics } from './main.js';
...
export async function createRouteFromMap() {
    ...
    if (getCitySlotUsage(Game.selectedStart.id, gameState.routes) >= startMaxSlots) {
        showToast(`No slots at ${Game.selectedStart.name}! Upgrade?`, true);
        return;
    }
    if (getCitySlotUsage(Game.selectedEnd.id, gameState.routes) >= endMaxSlots) {
...
export function startFlight(rid) {
    const r = gameState.routes.find(r => r.id === rid);
    if (!r || r.active) {
...
export async function completeFlight(rid) {
    ...
    let profit = computeFlightProfit(r, demand, gameState.fuelPrice);
    ...
    if (gameState.loanActive && profit > 0) {
        profit = applyLoanRepayment(profit);
    }
    
    gameState.money += profit;
    gameState.totalRevenue += profit;
    Game.mapNeedsUpdate = true;
    saveGame();
    refreshAll();
...
export async function removeRoute(rid) {
    ...
    gameState.routes = gameState.routes.filter(r => r.id !== rid);
    Game.mapNeedsUpdate = true;
    saveGame();
    refreshAll();
...
export function renderRoutes() {
    ...
    const html = gameState.routes.map(r => {
